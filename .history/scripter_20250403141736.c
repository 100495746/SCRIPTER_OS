...
void execute_simple_comand(char *argvv[], char *filev[], int background) {
    ...
    if (pid == 0) { // Proceso hijo
        ...
        if (argvv[0] != NULL && strcmp(argvv[0], "sort") == 0) {
            ...
            // Modificar los argumentos para forzar un comportamiento consistente
            // Añadir la opción -o para que sort escriba directamente al archivo
            if (filev[1] != NULL) {
                // Crear un nuevo array de argumentos con -o y el archivo de salida
                char *new_args[max_args];
                int i;
                for (i = 0; argvv[i] != NULL && i < max_args - 3; i++) {
                    new_args[i] = argvv[i];
                }
                new_args[i++] = "-o";
                new_args[i++] = filev[1];
                ...
                // Reemplazar los argumentos originales
                for (i = 0; i < max_args; i++) {
                    argvv[i] = new_args[i];
                }
                
                // Ya que usamos -o, no necesitamos la redirección
                filev[1] = NULL;
            }
        }
        ...
        if (pids[i] == 0) { // Proceso hijo
            ...
            if (args[0] != NULL && strcmp(args[0], "sort") == 0) {
                ...
                if (i == num_commands - 1 && filev[1] != NULL) {
                    // Solo para el último comando en el pipe
                    // Crear un nuevo array de argumentos con -o y el archivo de salida
                    char *new_args[max_args];
                    int j;
                    for (j = 0; args[j] != NULL && j < max_args - 3; j++) {
                        new_args[j] = args[j];
                    }
                    new_args[j++] = "-o";
                    new_args[j++] = filev[1];
