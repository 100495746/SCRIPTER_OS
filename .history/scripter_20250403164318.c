...
// Declaración de la función Child_process
void Child_process(int i, int num_commands, int pipes[max_commands - 1][2], char *args[max_args]) {
...
int procesar_linea(char *linea) {
    ...
    } else { // Ensure proper handling of else block
    ...
    if (pid == 0) { // Proceso hijo
        ...
        if (filev[0] != NULL) {
            ...
            if ((fd = open(filev[0], O_RDONLY)) < 0) {
                perror("open error");
                exit(1); // Ensure proper error handling
            }
            if (dup2(fd, 0) < 0) {
                perror("dup2 error");
                exit(1);
        ...
        // Ensure proper handling of parent process logic
        if (background == 0) {
...
void execute_piped_commands(char *commands[], int num_commands) {
    ...
    for (i = 0; i < num_commands; i++) {
        ...
        if (pids[i] == 0) { // Proceso hijo
            Child_process(i, num_commands, pipes, args);
        }
    }
    }
...
void Child_process(int i, int num_commands, int pipes[9][2], char *args[15]){
    if (i < num_commands - 1)
    {
...
void detect_zombies(pid_t parent_pid) {
    ...
    if (pid == 0) { // Proceso hijo
        ...
        close(fd[1]); // Ensure 'fd' is defined in the parent process
        close(fd[1]);
        
        snprintf(command, sizeof(command), "ps --ppid %d -o pid,stat,cmd | grep Z", parent_pid);
        execl("/bin/sh", "sh", "-c", command, (char *)NULL);
        perror("execl error"); // Ensure 'command' is defined and valid
        perror("execl error");
        exit(1);
    } else { // Proceso padre
        close(fd[1]);
...
int main(int argc, char *argv[]) {
    ...
    char buffer[1];
    int first = 1;
    while (read(fd, buffer, 1) > 0) { // Ensure 'fd' is properly defined and opened
